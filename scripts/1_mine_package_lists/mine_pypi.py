...
OUTPUT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'package-lists'))
...
def save_package_names_checkpoint(package_names):
    """Save package names checkpoint - saves packages to file."""
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
...
def mine_pypi_packages():
    ...
    if has_checkpoint and checkpoint_count > 0:
        ...
        try:
            ...
            response = fetch_with_retry(simple_index_url, timeout=120)
            if response is None or response.status_code != 200:
                print(f"Error downloading PyPI package names")
                return
